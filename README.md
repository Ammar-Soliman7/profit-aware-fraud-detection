...
> **Graduation project.** Trained and evaluated on the
> [IEEE-CIS Fraud Detection](https://www.kaggle.com/c/ieee-fraud-detection) dataset.
...
The economic model uses: a 2% profit rate on legitimate transactions, a 5× cost multiplier for
wrongly rejecting a good customer, a 3× multiplier for missed fraud, and a fixed cost per manual
...
(α = 0.95); the autoencoder is a minor supporting signal. The score is left **uncalibrated** on
purpose — the RL agent relies on the raw spread of the CatBoost score, and calibration was tested
and removed because it hurt generalisation from validation to test.
...
set number of reviewers × reviews per day). Under that budget it re-tunes a static base policy
...
to match the thesis. Details are in [`dashboard/README.md`](dashboard/README.md).

## Repository structure

```
notebooks/
└── fraud_rl_pipeline.ipynb     the full research pipeline (classifier → RL → capacity)

dashboard/                       the Streamlit app
├── dashboard.py                 the app itself
├── fraudcore/                   shared library: cost function, policy, metrics, export
├── regenerate_dashboard.py      rebuild the dashboard's data without retraining
├── notebook_export_cell.py      the export cell to paste at the end of the notebook
├── requirements.txt
└── README.md                    how the notebook ↔ dashboard contract works

data/                            instructions for getting the dataset (data not included)
```

> **Note:** the trained model files (the CatBoost model, autoencoder, and RL checkpoints) are
> not stored in this repo because of their size. They are produced by running the notebook.

## How to run

**Prerequisites:** Python 3.9+.

**1. Get the dataset** — see [`data/README.md`](data/README.md) (IEEE-CIS Fraud Detection from
Kaggle).

**2. Run the pipeline** — open `notebooks/fraud_rl_pipeline.ipynb` and run it top to bottom.
This trains the classifier, the PPO policy, and the capacity layer, and (via the export cell)
writes the four files the dashboard needs.
...
Python · CatBoost · TensorFlow/Keras (autoencoder) · PyTorch (PPO) · NumPy · pandas ·
Streamlit · Plotly

## Dataset

IEEE-CIS Fraud Detection (Kaggle). The raw data is **not** included here for size and licensing
reasons — download it from Kaggle directly. See [`data/README.md`](data/README.md).

## License

See [LICENSE](LICENSE). <!-- add your name to the LICENSE file -->

---

*Built as a graduation project. The full methodology, baselines, and analysis live in the
notebook.*
